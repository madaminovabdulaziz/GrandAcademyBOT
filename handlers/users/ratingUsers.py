...
async def showRatingToUsers(message: Message, state: FSMContext):
    await message.answer("Iltimos, test ID-ni kiriting\n\n⚠️<b>Diqqat!</b>\nReyting faqat:\n-Prezident maktab o'quvchilari\n-Abiturientlar uchun ko'rsatiladi!")
...
async def showF(message: Message, state: FSMContext):
    ...
    if test_bor:
        test_turi = await db.get_test_type(test_id)
        if test_turi == "specialPM" or test_turi == "specialA":
            ...
            await message.answer("Bu test block test emas!")
            await message.answer("Qayta kiriting!", reply_markup=ReplyKeyboardRemove())
            return
